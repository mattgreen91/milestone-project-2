  ...
  var methods = {
    init: function (options) {
      ...
      var hideItem = function (el) {
        ...
        switch (opts.animType) {
          ...
          case "fade":
            $("#" + topId)
              .delay(delayShow)
              .fadeIn(opts.animSpeed, function () {
                removeInHTML(el);
              });
            break;
          case "flip":
      ...
      var addInHTML = function (el, id) {
        el.children(".mgcard-show").html(inHtml[id]);
      };
      ...
      while (i < itemsNum) {
        ...
        $(this).append(
          '<div id="' +
            ...
            '</div></div><div id="quizy-mg-item-top' +
            j +
      ...
      if (opts.resultIcons) {
        ...
        var xMid =
          $(this).width() / 2 - $("div.quizy-mg-notification-fly").width() / 2;
        var yMid =
          $(this).height() / 2 -
          $("div.quizy-mg-notification-fly").height() / 2 -
          opts.itemsMargin / 2;
        $("div.quizy-mg-notification-fly").css({
          top: yMid + "px",
          left: xMid + "px",
        });
      }
      ...
      if (opts.gameSummary) {
        ...
        gameEl.append(
          '<div id="quizy-game-summary"><div class="gs-column" id="gs-column1">' +
        ...
        var xMid = gameEl.width() / 2 - $("div#quizy-game-summary").width() / 2;
        var yMid =
          gameEl.height() / 2 -
          $("div#quizy-game-summary").height() / 2 -
          opts.itemsMargin / 2;
        $("div#quizy-game-summary").css({
          top: yMid + "px",
          left: xMid + "px",
        });
        ...
        if (opts.replayButton) {
          $("#quizy-game-summary").append(
            '<div id="gs-replaybut">' + opts.replayButtonText + "</div>"
          );
        }

        // Appends the close button
        $("#quizy-game-summary").append(
          '<div id="gs-closebut">' + opts.closeButtonText + "</div>"
        );
